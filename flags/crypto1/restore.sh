#!/bin/bash
thisDir="/root/admin/flags/crypto1"
flag=`ssh crypto "cat /usr/local/bin/crypto1_owner.txt"`
level=`ssh crypto "cat /usr/local/bin/crypto1_count.txt"`
...
ssh crypto "echo '$level' > /usr/local/bin/crypto1_count.txt"
ssh crypto "echo '$flag' > /usr/local/bin/crypto1_owner.txt"
