#!/bin/bash
thisDir="/root/admin/flags/crypto2"
flag=`ssh crypto "cat /usr/local/bin/crypto2_owner.txt"`
level=`ssh crypto "cat /usr/local/bin/crypto2_count.txt"`
...
ssh crypto "echo '$level' > /usr/local/bin/crypto2_count.txt"
ssh crypto "echo '$flag' > /usr/local/bin/crypto2_owner.txt"
