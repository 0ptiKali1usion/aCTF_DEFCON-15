#!/bin/bash
thisDir="/root/admin/flags/crypto3"
flag=`ssh crypto "cat /usr/local/bin/crypto3_owner.txt"`
level=`ssh crypto "cat /usr/local/bin/crypto3_count.txt"`
...
ssh crypto "echo '$level' > /usr/local/bin/crypto3_count.txt"
ssh crypto "echo '$flag' > /usr/local/bin/crypto3_owner.txt"
