#!/bin/bash
thisDir="/root/admin/flags/crypto4"
flag=`ssh crypto "cat /usr/local/bin/crypto4_owner.txt"`
level=`ssh crypto "cat /usr/local/bin/crypto4_count.txt"`
...
ssh crypto "echo '$level' > /usr/local/bin/crypto4_count.txt"
ssh crypto "echo '$flag' > /usr/local/bin/crypto4_owner.txt"
