#!/bin/bash
thisDir="/root/admin/flags/crypto6"
flag=`ssh crypto "cat /usr/local/bin/crypto6_owner.txt"`
level=`ssh crypto "cat /usr/local/bin/crypto6_count.txt"`
...
ssh crypto "echo '$level' > /usr/local/bin/crypto6_count.txt"
ssh crypto "echo '$flag' > /usr/local/bin/crypto6_owner.txt"
